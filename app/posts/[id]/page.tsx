...
export default async function PostPage({ params }: { params: { id: string } }) {
  ...
  const canApprove = !!session?.authorId && session.authorId === post.authorId;
  const canManageDraft = !!session?.isAdmin || !!session?.isSuperAdmin;

  return <PostEditor post={post} author={author} brief={brief} canApprove={canApprove} canManageDraft={canManageDraft} />;
}
